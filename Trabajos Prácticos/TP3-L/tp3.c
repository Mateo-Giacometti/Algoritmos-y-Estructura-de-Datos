...
node_t *node_new(const char* key, void *value) {
  ...
  if (!new_node->key) {
    free(new_node); // Liberar el nodo si la asignación de memoria falla
    return NULL;
  }
...
dictionary_t *dictionary_create(destroy_f destroy) { 
  ...
  if (!dic) return NULL;
  dic->buckets = malloc(sizeof(list_t *) * 10);
  if (!dic->buckets) {
    free(dic);
    return NULL;
  }
  for (int i = 0; i < 10; i++) {
    ...
    if (!dic->buckets[i]) {
      for (int j = 0; j < i; j++) {
        list_destroy(dic->buckets[j], destroy);
      }
      free(dic->buckets);
      free(dic);
      return NULL;
...
void *dictionary_pop(dictionary_t *dictionary, const char *key, bool *err) {
  ...
  if(strcmp(list_iter_peek_current_key(iter), key) == 0){
    void *value = list_iter_delete(iter);
    list_iter_delete(iter);
    list_iter_destroy(iter); //preguntar si esta bien 
    dictionary->size--;
    *err = false;
    return value;
  }
...
void dictionary_destroy(dictionary_t *dictionary){
  for(int i = 0; i < dictionary->capacity; i++){
    list_destroy(dictionary->buckets[i], dictionary->destroy);
  }
